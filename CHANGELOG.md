...
## [Unreleased]
### Added
- **`/brainstorm-save` Skill + AI Collaboration Model (`TSK-044`)**: Added the `/brainstorm-save` **project** skill (`.kiro/skills/`) — curates a session's key decisions into a contributor-attributed entry appended to `docs/DECISION-JOURNAL.md` (append-only, credibility-first). Added `docs/ai-collaboration-model.md` — the methodology one-pager describing the human-directed / AI-assisted / human-gated loop (Kiro Web plan ↔ IDE execute, Specs as agent contracts, progressive formality, AIDLC on Kiro-native Autonomous mode). Added contributor attribution to the Decision Journal and appended Entry 002 (sprint-lifecycle / AIDLC / decision-capture decisions). Clarified that `/sign-off` should *prompt* (never auto-run) `/brainstorm-save`.
- **Decision Journal (`docs/DECISION-JOURNAL.md`, `TSK-043`)**: Added a curated decision-narrative log capturing *how* key engineering decisions are reached (problem → options → rationale → outcome) — distinct from the CHANGELOG (*what* shipped) and RFCs (*formal* decisions). Seeded with a comprehensive inaugural entry covering the foundational session (cloud-dev migration → relational data → PocketBase web app → branch-based dev → work-environment standardization → MVP scoping → Kiro-native Specs/AIDLC), including the human-directed/AI-assisted working method, course-corrections, and honest failure/recovery notes.
- **MVP Scoping — Definition & Minimum RBAC (`RFC-LAB-000-008`, `BK-011`, `TSK-042`)**: Authored the MVP scoping RFC atop the now-verified local stack. **MVP** = logged-in user sees the live PocketBase portfolio; a project **owner edits their own project's status**; deployed. **Minimum RBAC** = 3 tiers (public read / authenticated detail / owner-writes-own via the 1:1 `owner_id`), explicitly deferring the full 5-role `memberships` model, audit trail, and `status_snapshots` migration. Includes a phase breakdown (M1 wire UI→PocketBase → M5 deploy), acceptance criteria, and open questions.
- **`make setup` Staleness Guard**: Added a non-blocking "step 0" to `scripts/setup-local.sh` that prints the current commit and warns (never auto-pulls) when the local checkout is behind `origin` — preventing the "ran stale code" trap.
- **Programmatic PocketBase Provisioning (`TSK-041`)**: Added `scripts/pb_provision.py` (`make provision`) — creates the `users`/`projects`/`memberships` collections via the PocketBase REST API in dependency order, resolving real relation ids at create time. This replaces the fragile hand-written schema-JSON import (which fails on PocketBase v0.40, confirmed locally — the importer requires real generated collection ids and the exact export shape). Idempotent (updates existing collections in place). Wired into `make setup` and `make seed` so first-run local setup has **zero manual UI steps**. `pb_schema.json` is now a human-readable reference (not an import artifact); its generator note and `app/pocketbase/README.md` updated accordingly.
- **One-Time Local Setup Script (`TSK-040`)**: Added `scripts/setup-local.sh` (`make setup`) — an idempotent, one-command bootstrap for local development: toolchain check, `.env` creation with sensible defaults, web-dependency install, PocketBase superuser upsert (compliant ≥8-char password), collections-schema import guidance, and an optional `data/` seed. BSD/macOS-`sed` compatible; re-runnable. Documented in `docs/work-environment.md` §3.
- **Work-Environment Standardization (`RFC-LAB-000-007`, `TSK-039`)**: Standardized the Kiro Web + IDE developer experience — explicit **surface roles** (Web = stateless governance/docs; IDE = stateful servers), `.env.example`, and a one-way personal config-sync protocol. Added a **`Makefile`** (`make help` cheat-sheet mirroring the `/commands`), a **Podman-first `Containerfile`** for PocketBase (Docker fallback; staging/GCP parity — local dev stays binary-first), the **`/env-doctor`** surface-aware readiness skill (diagnostic-only, delegates to `/validate-local`), and `docs/work-environment.md` (adapted from the Nexus Pulse `LAB-003` guide). `/validate-local` and `/validate-staging` cross-reference the `/env-doctor` boundary.
### Changed
